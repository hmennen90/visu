...
class Input implements WindowEventHandlerInterface, InputInterface
{
    ...
    /**
     * Callback-tracked mouse button states. Updated by handleWindowMouseButton().
     * Used instead of glfwGetMouseButton polling which is unreliable in macOS fullscreen.
     *
     * @var array<int, int> button => GLFW_PRESS|GLFW_RELEASE
     */
    ...
    /**
     * When > 0, mouse button and key callbacks are ignored (decremented each endFrame).
     * Used to suppress spurious events after display mode changes (fullscreen toggle).
     */
    private int $suppressInputEventsFrames = 0;

    /**
     * Time-based suppression: ignore input events until this timestamp.
     * Complements frame-based suppression to catch late phantom events on macOS.
     */
    ...
    {
        // Use callback-tracked state which works reliably in both windowed and fullscreen.
        // glfwGetMouseButton polling is unreliable in macOS fullscreen mode.
        return $this->mouseButtonStates[$button] ?? GLFW_RELEASE;
    }
    ...
    {
        // Ignore spurious events generated by display mode changes (fullscreen toggle)
        if ($this->suppressInputEventsFrames > 0 || microtime(true) < $this->suppressInputUntil) {
            return;
        }
     ...
     */
    public function suppressInputEvents(int $frames = 3, float $seconds = 0.5): void
    {
